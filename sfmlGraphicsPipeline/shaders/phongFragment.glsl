...
vec3 computeSpotLight(SpotLight light, vec3 surfel_to_camera)
{
    ...
    //float intensity = dot(surfel_to_light, (-light.spotDirection)) < light.innerCutOff ? 0.0 : 1.0;
    float intensity = clamp( (dot(surfel_to_light, (-light.spotDirection)) - light.outerCutOff)/(light.innerCutOff - light.outerCutOff), 0, 1);
